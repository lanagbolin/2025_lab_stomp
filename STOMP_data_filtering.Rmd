...
# These are different plants in a quad
data %>% 
  group_by(species, quadID, year, season) %>%
  dplyr::summarise(n = n()) %>%
  filter(n > 1)

# Calculate total cover for all individuals in a quad
data$obscover <- data$cover * data$count 
data$quad_id <- as.factor(paste(data$site, data$treatment, data$plot, data$quad, sep="_")) # This quad_id leaves out things like web and block that aren't relevant for stomp data

catch <- data %>% 
  filter(quad_id == "crust_grass_C_25_1") %>%
  filter(year == 2018) %>%
  filter(season == 2) %>%
  filter(species == "BOER4")
...
data.wide <- dcast(quadcover, year + season + site + treatment + plot + quad + quad_id ~ species, sum, value.var = "quadcover", fill=0)
  ...
  # Checking if a quad was empty before removing these (gut check for a question in "STOMP_data_merging.RMD")
  data.wide %>% filter(quad_id == "crust_creosote_C_20_3" & year == 2020)

# Remove "EMPTY" after casting
...
data.long %>% filter(species == "CHAMA15")

### Write long format
write.csv(data.long, "~/GitHub/2025_lab_stomp/data/data.stomp.2018-2023.long.csv", row.names = F)
```

Testing that I can read the sheet in
```{r, message=F, eval=F}
library(googlesheets4)

test.read <- read_sheet("https://docs.google.com/spreadsheets/d/1Al3Nun5yWGBwM6rUYVQiUKRvDOq3EJSkLTElWZ4O-zI/edit?gid=1087477164#gid=1087477164")
